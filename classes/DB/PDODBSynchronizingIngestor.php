...
class PDODBSynchronizingIngestor implements Ingestor
{
     ...
     * @param mixed $uniqColumns Either a single primary key column or
     *   an array of columns that can be used to check for duplicates.
    ...
    private function getKeyFromRow(array $row)
    {
        ...
        $escapedKeyValues = array_map(
            function ($key) {
                return str_replace("\0", "\0\0", $key);
            },
            $keyValues
        );

        return implode("\0", $escapedKeyValues);
    }
}
