...
class Builder extends \aRestAction
{
   ...
   private function getImageVisibility() {
   ...
   private function getImageAction()
   {
                  
      $user = $this->_authenticateUser();

      $actionParams = $this->_parseRestArguments('chart_identifier');
      
      $output_format = $this->_getRawFormat();
                        
      //todo: $output_format->toString() should eventually be used in generating the chart
      
      $chart = \xd_charting\getChartFromURI($actionParams['chart_identifier'], $user);
   ...
   private function getImageDocumentation()
   {
      
      $documentation = new \RestDocumentation();
      
      $documentation->setDescription('Generate chart data.');

      $documentation->setAuthenticationRequirement(true);
      
      $documentation->addArgument('chart_id', 'A unique identifier associated with a particular chart');
      
      $documentation->addReturnElement("RAW", "A chart in graphic format");
