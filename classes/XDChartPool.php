	...
	class XDChartPool {
		...
		public function __construct($user) {
         ...
         $this->_pdo = DB::factory('database');
			...
			//$this->_site_host = str_replace('https://', 'http://', $this->_site_host);
			
			// The XML definition will be used internally, so the report builder will need to make REST calls internally...
			
			$this->_site_host = preg_replace('/(http|https):([^:\s]*)((:\d*)|)/', "http://127.0.0.1$3", $this->_site_host);
      ...
      public function emptyCache() {
		...
		public function addChartToQueue($chartIdentifier, $chartTitle, $chartDrillDetails, $chartDateDesc) {
         ...
         // Since we are now letting the user have full control over the titles of charts (courtesy of the Usage Explorer),
