...
{	public function __construct()
	...
	public function applyTo(\DataWarehouse\Query\Query &$query, \DataWarehouse\Query\Model\Table $data_table, $multi_group = false)
	{
		...
		$persontable_id_field = new \DataWarehouse\Query\Model\TableField($this->person_table,'id');
		...
		$persontable_nsfstatuscode_id_field = new \DataWarehouse\Query\Model\TableField($this->person_table,'nsfstatuscode_id');
		$query->addWhereCondition(new \DataWarehouse\Query\Model\WhereCondition($nsfstatus_id_field,
													'=',
													$persontable_nsfstatuscode_id_field
													));			
	...
	public function pullQueryParameters(&$request)
	{
		...
		if(isset($request[$this->getName().'_filter']) && $request[$this->getName().'_filter'] != '')
		{
			...
			if(count($filterItems) > 0) $parameters[] = new \DataWarehouse\Query\Model\Parameter('person_nsfstatuscode_id', 'in', "(".implode(',',$filterItems).")");		
		}
		else
		if(isset($request[$this->getName()]))
		{
			$parameters[] = new \DataWarehouse\Query\Model\Parameter('person_nsfstatuscode_id', '=', $request[$this->getName()]);		
