...
class DataWarehouseInitializer
{
    ...
    /**
     * @param Database $shredderDb The shredder database.
     * @param Database $hpcdbDb The HPcDB database.
     * @param Database $warehouseDb The MoD warehouse database.
     */
     ...
     * @param Database $destDb The destination database.
     * @param string $startDate
     * @param string $endDate
     */
    ...
    public function initializeAggregation($startDate = null, $endDate = null)
    {
        ...
        if (count($resourcesWithoutInfo) > 0) {
            ...
            $msg = 'New Resource(s) in resourcefact table do not have '
        ...
        $generator = new TimeseriesGenerator('year');
        $generator->execute($this->warehouseDb, $this->aggDbName);

        $generator = new TimeseriesGenerator('quarter');
        $generator->execute($this->warehouseDb, $this->aggDbName);

        $generator = new TimeseriesGenerator('month');
        $generator->execute($this->warehouseDb, $this->aggDbName);

        $generator = new TimeseriesGenerator('day');
        $generator->execute($this->warehouseDb, $this->aggDbName);
    }
    ...
    public function aggregateAllJobs(
        ...
        $aggregator = new JobTimeseriesAggregator('year');
        ...
        $aggregator = new JobTimeseriesAggregator('quarter');
        $aggregator->setLogger($this->logger);
        $aggregator->execute(
            $this->warehouseDb,
            $this->aggDbName,
        ...
        $aggregator = new JobTimeseriesAggregator('month');
        $aggregator->setLogger($this->logger);
        $aggregator->execute(
            $this->warehouseDb,
            $this->aggDbName,
        ...
        $aggregator = new JobTimeseriesAggregator('day');
        $aggregator->setLogger($this->logger);
        $aggregator->execute(
            $this->warehouseDb,
            $this->aggDbName,
