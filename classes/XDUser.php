...
class XDUser {
   ...
   */
   ...
   function __construct($username = NULL, $password = NULL, $email_address = NO_EMAIL_ADDRESS_SET,
      ...
      $userCheck = $this->_pdo->query("SELECT id FROM Users WHERE username='$username'");
   ...
   */
   ...
   public static function userExistsWithUsername($username) {
      ...
      $userCheck = $pdo->query("SELECT id FROM Users WHERE username='$username'");
   ...
   */
   ...
   public static function getUserByToken ($token) {
      ...
      $userCheck = $pdo->query("SELECT id FROM Users WHERE token LIKE BINARY '$token'");
   ...
   public static function getUserByID($uid, &$targetInstance = NULL) {
      ...
      $user->_user_type = $userCheck[0]['user_type'];

      $user->_roles = array();

      $rolesResult = $pdo->query("
         SELECT r.abbrev, r.description, ur.is_primary, ur.is_active
         FROM UserRoles AS ur, Roles AS r
         WHERE ur.user_id='{$user->_id}' AND ur.role_id = r.role_id
      ");
   ...
   public static function authenticate($uname, $pass) {
      ...
      $userCheck = $pdo->query("SELECT id FROM Users WHERE username=:username AND password=MD5(:password)", array('username' => $uname, 'password' => $pass));
   ...
   public function saveUser() {
      ...
      $update_data = array();
      $update_data['username'] = xd_utilities\quote($this->_username);

      if (strlen($this->_password) <= CHARLIM_PASSWORD) {
         $update_data['password'] = xd_utilities\quote(md5($this->_password));
         $update_data['password_last_updated'] = 'NOW()';
         //$this->_timePasswordUpdated = $update_data['password_last_updated'];
      }
      ...
      $update_data['email_address'] = xd_utilities\quote($this->_email);
      $update_data['first_name'] = xd_utilities\quote($this->_firstName);
      $update_data['middle_name'] = xd_utilities\quote($this->_middleName);
      $update_data['last_name'] = xd_utilities\quote($this->_lastName);

      $update_data['field_of_science'] = xd_utilities\quote($this->_field_of_science);

      $update_data['organization_id'] = $this->_organizationID == NULL ? 'NULL' : xd_utilities\quote($this->_organizationID);
      $update_data['person_id'] = $this->_personID == NULL ? 'NULL' : xd_utilities\quote($this->_personID);

      $update_data['user_type'] = $this->_user_type;

      //$update_data['time_created'] = $this->_timeCreated;
      //$update_data['time_last_updated'] = date('Y-m-d H:i:s');
      //$update_time = $update_data['time_last_updated'];

      if ($this->_update_token) {
         $update_data['token'] = xd_utilities\quote($this->_generateToken());
      ...
      if (isset($this->_id)) {
         ...
         foreach($update_data as $field => $value) {
            $update_data[$field] = $field."=".$value;
         }

         $query .= implode(', ', $update_data).' ';
         ...
         //print $query;
         //$query = "INSERT INTO Users (id, $columns) SELECT (SELECT MAX(id) + 1 FROM Users LIMIT 1 OFFSET 0) AS id, $values";

      }

      // NOTE: There may be a better way to do this (atomicity issue) ?
      $new_user_id = $this->_pdo->insert($query);
      ...
      if ($this->_update_token) {
         ...
         $this->_pdo->execute("UPDATE Users SET token_expiration=DATE_ADD(NOW(), INTERVAL 30 DAY) WHERE id={$this->_id}");
         $this->_update_token = false;
      }

      // Rebuild roles data for user --------------

      $this->_pdo->execute("DELETE FROM UserRoles WHERE user_id='{$this->_id}'");

      foreach($this->_roles as $role){
         $this->_pdo->execute("INSERT INTO UserRoles VALUES ('{$this->_id}', '".$this->_getRoleID($role)."', '0', '0')");
      }

      $this->_pdo->execute("UPDATE UserRoles SET is_primary='1' WHERE user_id='{$this->_id}' AND role_id='".$this->_getRoleID($this->_primary_role->getIdentifier())."'");
      ...
      $active_role_id  = ( in_array($this->_active_role->getIdentifier(), $this->_roles) ) ?
                           ...
                           $this->_getRoleID($this->_primary_role->getIdentifier());


      $this->_pdo->execute("UPDATE UserRoles SET is_active='1' WHERE user_id='{$this->_id}' AND role_id='$active_role_id'");

      $timeResult = $this->_pdo->query("SELECT time_last_updated FROM Users WHERE id=".$this->_id);

      $this->_timeUpdated = $timeResult[0]['time_last_updated'];


      $this->_primary_role->configure($this);
      $this->_active_role->configure($this);

      //$this->_timeUpdated = $update_time;
   ...
   public function getLastLoginTimestamp() {
      ...
      $results = $this->_pdo->query("SELECT init_time FROM SessionManager WHERE user_id='{$this->_id}' ORDER BY init_time DESC LIMIT 1");
   ...
   public function getToken() {
      ...
      $tokenResults = $this->_pdo->query("SELECT token FROM Users WHERE id='{$this->_id}'");
   ...
   public function getTokenExpiration() {
      ...
      $tokenResults = $this->_pdo->query("SELECT token_expiration FROM Users WHERE id='{$this->_id}'");
   ...
   private function _getRoleID($role_abbrev) {
      ...
      //print "SELECT role_id FROM Roles WHERE abbrev='$role_abbrev'";

      $roleResults = $this->_pdo->query("SELECT role_id FROM Roles WHERE abbrev='$role_abbrev'");
   ...
   public function removeUser() {
      ...
      $this->_pdo->execute("DELETE FROM ChartPool WHERE user_id='{$this->_id}'");
      $this->_pdo->execute("DELETE FROM Reports WHERE user_id='{$this->_id}'");
      $this->_pdo->execute("DELETE FROM ReportCharts WHERE user_id='{$this->_id}'");

      $this->_pdo->execute("DELETE FROM UserRoleParameters WHERE user_id='{$this->_id}'");
      $this->_pdo->execute("DELETE FROM UserRoles WHERE user_id='{$this->_id}'");

      // Reset any associations to dependent users
      $this->_pdo->execute("UPDATE UserRoleParameters SET promoter='-1' WHERE promoter='{$this->_id}'");

      $this->_pdo->execute("DELETE FROM Users WHERE id='{$this->_id}'");
   ...
   public function enumAllAvailableRoles() {
      ...
      $role_query_1 = "SELECT r.description, r.abbrev AS param_value, urp.is_primary, urp.is_active " .
                      ...
                      "WHERE r.role_id = urp.role_id AND user_id={$this->_id} " .
      ...
      $role_query_2 = "SELECT CONCAT(r.description, ' - ', o.abbrev) AS description, CONCAT(r.abbrev, ':', urp.param_value) AS param_value, urp.is_primary, urp.is_active " .
                      ...
                      "WHERE urp.param_value = o.id AND r.role_id = urp.role_id AND urp.user_id={$this->_id} AND r.description != 'Campus Champion'" .
      ...
      $role_query_3 = "SELECT CONCAT(r.description, ' - ', o.name) AS description, CONCAT(r.abbrev, ':', urp.param_value) AS param_value, urp.is_primary, ur.is_active " .
                      ...
                      "WHERE urp.param_value = o.id AND ur.role_id = r.role_id AND ur.user_id ={$this->_id} AND r.role_id = urp.role_id " .
                      "AND urp.user_id={$this->_id} AND r.description = 'Campus Champion'" .
                      "ORDER BY r.description, o.abbrev";
      ...
      $available_roles = array_merge(
         $this->_pdo->query($role_query_1),
         $this->_pdo->query($role_query_2),
         $this->_pdo->query($role_query_3),
         $this->_pdo->query($role_query_4),
         $this->_pdo->query($role_query_5)
      );
   ...
   public function setInstitution($institution_id, $is_primary = false) {
      ...
      $cleanupStatement = "DELETE FROM UserRoleParameters " .
                          "WHERE user_id={$this->_id} AND role_id=6 AND param_name='institution'";
      ...
      $insertStatement = "INSERT INTO UserRoleParameters " .
                         ...
                         "VALUES ({$this->_id}, 6, 'institution', '=', $institution_id, $primary_flag, 1, -1)";

      $this->_pdo->execute($cleanupStatement);
      $this->_pdo->execute($insertStatement);
   ...
   private function _getActiveProvider($role_id) {
      ...
      $active_provider = $this->_pdo->query("SELECT param_value FROM UserRoleParameters " .
                                            "WHERE user_id={$this->_id} AND role_id=$role_id AND param_name='provider' AND is_active=1");
   ...
   public function getActiveRoleSettings() {
      ...
      $mainRole = $this->_pdo->query("SELECT r.abbrev, r.role_id FROM Roles AS r, UserRoles AS ur WHERE r.role_id = ur.role_id AND ur.user_id={$this->_id} AND ur.is_active=1");
      ...
      if ($mainRole == ROLE_ID_CENTER_DIRECTOR || $mainRole == ROLE_ID_CENTER_STAFF) {
         ...
         $activeCenter = $this->_pdo->query("SELECT param_value FROM UserRoleParameters WHERE user_id={$this->_id} AND role_id=$mainRoleID AND is_active=1");
   ...
   public function setOrganizations($organization_ids = array(), $role = ROLE_ID_CENTER_DIRECTOR, $reassignActiveToPrimary = false) {
      ...
      $this->_pdo->execute("DELETE FROM UserRoleParameters " .
                           "WHERE user_id={$this->_id} AND role_id=$role_id AND param_name='provider'");
      ...
      foreach ($organization_ids as $organization_id => $config) {
         ...
         $insertStatement = "INSERT INTO UserRoleParameters " .
                            "(user_id, role_id, param_name, param_op, param_value, is_primary, is_active, promoter) " .
                            "VALUES ({$this->_id}, $role_id, 'provider', '=', $organization_id, $primary_flag, $active_flag, -1)";
   ...
   public function enumCenterStaffSites() {
      ...
      $sites = $this->_pdo->query("SELECT param_value AS provider, is_primary FROM moddb.UserRoleParameters WHERE role_id=5 AND param_name='provider' AND user_id={$this->_id}");
   ...
   public function enumCenterDirectorSites() {
      ...
      $sites = $this->_pdo->query("SELECT param_value AS provider, is_primary FROM moddb.UserRoleParameters WHERE role_id=1 AND param_name='provider' AND user_id={$this->_id}");
   ...
   public function disassociateWithOrganizations() {
      ...
      $cleanupStatement = "DELETE FROM UserRoleParameters WHERE user_id={$this->_id} AND role_id=1 AND param_name='provider'";
   ...
   public function getInstitution() {
      ...
      $query = "SELECT param_value FROM UserRoleParameters WHERE user_id={$this->_id} AND param_name='institution'";
   ...
   public function getPrimaryOrganization() {
      ...
      $query = "SELECT urp.param_value FROM UserRoleParameters AS urp, Roles AS r WHERE urp.role_id = r.role_id AND r.abbrev='cd' AND urp.user_id={$this->_id} AND urp.param_name='provider' AND urp.is_primary=1";
   ...
   public function getActiveOrganization() {
      ...
      $query = "SELECT urp.param_value FROM UserRoleParameters AS urp, Roles AS r WHERE urp.role_id = r.role_id AND r.abbrev='cd' AND urp.user_id={$this->_id} AND urp.param_name='provider' AND urp.is_active=1";
   ...
   public function getOrganizationCollection($center_staff_or_director = ROLE_ID_CENTER_STAFF) {
      ...
      $query = "SELECT urp.param_value FROM UserRoleParameters AS urp, Roles AS r WHERE urp.role_id = r.role_id AND r.abbrev='$center_staff_or_director' AND urp.user_id={$this->_id} AND urp.param_name='provider'";
   ...
   public function getRoles($flag = 'informal') {
      ...
      if ($flag == 'formal') {
         ...
         $query .= "WHERE r.role_id = ur.role_id AND ur.user_id = '".$this->_id."' ORDER BY ur.is_primary DESC";
   ...
   */
   ...
   private function _getRoleIDFromIdentifier($identifier) {
      ...
      $role_data = $this->_pdo->query("SELECT role_id FROM Roles WHERE abbrev='$identifier'");
   ...
   public function setActiveRole($active_role, $role_param = NULL) {
      ...
      if ($active_role == ROLE_ID_CENTER_DIRECTOR || $active_role == ROLE_ID_CENTER_STAFF) {
         ...
         if ($this->_isValidOrganizationID($role_id, $role_param) == true) {
            ...
            $this->_pdo->execute("UPDATE moddb.UserRoleParameters SET is_active=0 WHERE user_id={$this->_id} AND role_id != $campus_champion_role_id");
            $this->_pdo->execute("UPDATE moddb.UserRoleParameters SET is_active=1 WHERE user_id={$this->_id} AND role_id=$role_id AND param_value=$role_param");
      ...
      $this->_pdo->execute("UPDATE moddb.UserRoles SET is_active=0 WHERE user_id={$this->_id}");
      $this->_pdo->execute("UPDATE moddb.UserRoles SET is_active=1 WHERE user_id={$this->_id} AND role_id=$role_id");
   ...
   */
   ...
   public function assignActiveRoleToPrimary() {
      ...
      $this->_pdo->execute("UPDATE moddb.UserRoles SET is_active=is_primary WHERE user_id={$this->_id}");
      $this->_pdo->execute("UPDATE moddb.UserRoleParameters SET is_active=is_primary WHERE user_id={$this->_id}");
   ...
   */
   ...
   public  function _getFormalRoleName($role_abbrev) {
      ...
      $roleData = $pdo->query("SELECT description FROM Roles WHERE abbrev='$role_abbrev'");
   ...
   */
   ...
   private function _generateToken() {
      ...
      $results = $this->_pdo->query("SELECT * FROM Users WHERE token='$string'");
   ...
   */
   ...
   public static function validateRID($rid) {
      ...
      $accountCheck = $pdo->query("SELECT id, first_name FROM Users WHERE MD5(CONCAT(username, password_last_updated)) = '$rid'");
   ...
   public function getDisabledMenus($realms)
   {
      ...
      foreach($this->_roles as $role_abbrev)
      {
         ...
         $returnData[$role_abbrev] = $role->getDisabledMenus($realms);

      }
      $role = \User\aRole::factory($this->_getFormalRoleName('pub'));
      $returnData['pub'] = $role->getDisabledMenus($realms);
      return $returnData;
   }
}//XDUser
