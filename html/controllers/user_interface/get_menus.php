...
try
{
   ...
   $user = \xd_security\detectUser(array(XDUser::PUBLIC_USER)); 
	...
	if(isset($_REQUEST['node']) && $_REQUEST['node'] == 'realms')
	{
		...
		$realms = array();
		$realms = array_keys($activeRole->getAllQueryRealms($query_group_name));
		
		foreach($realms as $realm)
		{
			$returnData[] = array('text' => $realm,
				'id' => 'realm_'.$realm, 
				'realm' => $realm,
	...
	else if(isset($_REQUEST['node']) && substr($_REQUEST['node'],0,6) == 'realm_')
	{
		if(isset($_REQUEST['realm']))
		{
			...
			{
				$query_descripter_groups = $activeRole->getQueryDescripters($query_group_name, $realm_name);
				...
				foreach($query_descripter_groups as $query_descripter_group)
				{
					...
					{		
						...
						$returnData[] = array('text' => $query_descripter->getMenuLabel(),
											'id' => 'group_by_'.$realm_name.'_'.$query_descripter->getGroupByName(), 
											...
											'node_type' => 'group_by',
											'iconCls' => 'menu', 
											//'disabled' => $query_descripter->getDisableMenu(),
	...
	else if(isset($_REQUEST['node']) && substr($_REQUEST['node'],0,9) == 'group_by_')
	{
		if(isset($_REQUEST['realm']))
		{
			...
			if(isset($_REQUEST['group_by']))
			{
				...
				{		
					...
					{
						$returnData[] = array('text' => $statistic_object->getLabel(false),
							'id' => 'statistic_'.$realm_name.'_'.$group_by_name.'_'.$statistic_object->getAlias()->getName(), 
