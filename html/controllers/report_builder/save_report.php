   ...
   try {
      ...
      foreach($_POST as $k => $v) {
         ...
         if (preg_match('/chart_data_(\d+)/', $k, $m) > 0) {
            ...
            // Transfer blobs residing in /tmp into the database as necessary for each chart which comprises the report.
            
            if (isset($_POST[$cache_ref_variable])) {
            
               list($start_date, $end_date, $ref, $location) = explode(';', $_POST[$cache_ref_variable]);
   
               $location = "/tmp/{$ref}_{$location}_{$start_date}_{$end_date}.png";
