...
try
{
   ...
   $ak_db = new \AppKernel\AppKernelDb();
	...
	if(isset($_REQUEST['node']) && $_REQUEST['node'] === 'resources') 
	{
		...
		$resources = $ak_db->getResources($_REQUEST['start_date'],$_REQUEST['end_date'], 
	...
	if(isset($_REQUEST['node']) && $_REQUEST['node'] === 'pus') 
	{
		...
		$processing_units = $ak_db->getProcessingUnits('2010-01-01',date("Y-m-d"),  
	...
	if(isset($_REQUEST['node']) && $_REQUEST['node'] === 'app_kernels') 
	{
		...
		{
			$metrics = $ak_db->getMetrics($app_kernel->id,'2010-01-01',date("Y-m-d")/*, 
										$selectedResourceIds, $selectedProcessingUnits*/);
