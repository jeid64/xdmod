<?php
   ...
   session_write_close();
	...
	// This controller accepts internal tokens so the STATUS_LOGGED_IN check qualifies when this
	// controller is accessed internally with a valid (recognizable) token appended to the query
	
	$controller->acceptInternalTokens();
	
	$controller->registerOperation('get_data');	
	$controller->registerOperation('get_queries');
