<?php
	...
	{		
		...
		if( is_array($_REQUEST['data_series']) && is_array($_REQUEST['data_series']['data']) )   
		{
			...
			foreach($v as $x)
			{
				...
				if(!isset($y->line_width) || empty($y->line_width) || !is_numeric($y->line_width))
				{
					$y->line_width = 2;
				}
	...
	{
		return isset($_REQUEST['showContextMenu'])?$_REQUEST['showContextMenu'] == 'true' || $_REQUEST['showContextMenu'] === 'y': false;
	}
?>