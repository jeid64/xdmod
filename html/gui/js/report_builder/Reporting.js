...
XDMoD.Reporting.chartDetailsRenderer = function(val, metaData, record, rowIndex, colIndex, store){
   ...
   */
   ...
   if (store.storeId == 'report_generator_included_charts_store') {
	  ...
	  chart_entry_timeframe = '<div style="margin-top: 2px"><a href="javascript:void(0)" onClick="XDMoD.Reporting.Singleton.ChartDateEditor.present(' + editorConfig + ', \'' + [store.storeId, record.id].join('\', \'') + '\')">' + 
...
CCR.xdmod.ReportCheckbox = Ext.extend(Ext.form.Checkbox,  {
   ...
   storeChartArguments: function(chart_args, title, drill_details, start_date, end_date, included_in_report) {
      ...
      var escaped_chart_args = chart_args.replace(title, escape(title).replace(/@/g, '%40').replace(/\*/g, '%2A').replace(/\//g, '%2F').replace(/\+/g, '%2B').replace(/%20/g, '+'));
   ...
   toggleReportInclusion: function (checkbox, checked) {
      ...
      var cb_module = checkbox.module ? checkbox.module : 'usage_explorer';
      ...
      conn.request({
         ...
         callback: function (options, success, response)
         {
            ...
            if (success)
            {
               ...
               {
                  Ext.MessageBox.alert('Report Builder', 'There was a problem managing the report queue.<br>(' + json.message + ')');
               }
               
            }
            else
            {
               Ext.MessageBox.alert('Report Builder', 'There was a problem managing the report queue.');
            }
