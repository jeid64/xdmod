...
Ext.ux.Printer.BaseRenderer = Ext.extend(Object, {
  ...
  print: function(component) {
    /* var name = component && component.getXType
             ? String.format("print{1}", component.getXType(), component.id)
             : "print";*/
   ...
   */
  stylesheetPath: 'gui/css/viewer.css'
});
 ...
 * Prints the contents of an Ext.Panel
 */
Ext.ux.Printer.DataViewRenderer = Ext.extend(Ext.ux.Printer.BaseRenderer, {
 ...
 generateBody: function(view) {
   ...
   return String.format("<div class='x-panel-print'>{0}</div>",view.el.dom.innerHTML);
 }
});

Ext.ux.Printer.registerRenderer("dataview", Ext.ux.Printer.DataViewRenderer);


Ext.ux.Printer.HTMLRenderer = Ext.extend(Ext.ux.Printer.BaseRenderer, {
 generateBody: function(html) {
	
   return String.format("<div class='x-panel-print'>{0}</div>",html.html);
 }
});

Ext.ux.Printer.registerRenderer("html", Ext.ux.Printer.HTMLRenderer);