...
Series.prototype = {
	 ...
	 */
	  /*  function changed by Amin Ghadersohi 10/30 to support dashed connected null points */
	...
	/*
	 function added by Amin Ghadersohi 10/30 to support dashed connected null points
	*/
	getMissingGraphPath: function () {
		var series = this,
			segmentPath,
			missingGraphPath = []; // used in drawTracker
		if(series.missingSegments)
		{
			// Divide into segments and build graph and area paths
	...
	drawGraph: function () {
		...
		props = [['missingGraph', options.lineColor || this.color]];
		// draw the graph for missing segments
		each(props, function (prop, i) {
			var graphKey = prop[0],
				graph = series[graphKey],
				attribs;
			...
			} else if (lineWidth && missingGraphPath.length) { // #1487
				attribs = {
					stroke: prop[1],
					'stroke-width': lineWidth,
					zIndex: 1 // #1069
