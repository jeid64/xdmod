<?php
   ...
   function getRoleIDFromIdentifier($identifier) {
      ...
      $role_data = $pdo->query("SELECT role_id FROM Roles WHERE abbrev='$identifier'");
   ...
   function getFormalRoleNameFromIdentifier($identifier) {
      ...
      $role_data = $pdo->query("SELECT description FROM Roles WHERE abbrev='{$role_config[0]}'");
      ...
      switch($role_config[0]) {
         ...
         case ROLE_ID_CAMPUS_CHAMPION:
            ...
            $role_query = $pdo->query("SELECT o.name FROM modw.organization AS o WHERE o.id={$role_config[1]}");
         ...
         case ROLE_ID_CENTER_STAFF:
            ...
            $role_query = $pdo->query("SELECT CONCAT(o.abbrev, ' (', o.name, ')') AS description FROM modw.organization AS o WHERE o.id={$role_config[1]}");
